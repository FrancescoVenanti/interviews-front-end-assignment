...
function useVisualElement(Component3, visualState, props, createVisualElement2) {
  ...
  const wantsHandoff = (void 0)(Boolean(props[optimizedAppearDataAttribute] && !window.HandoffComplete));
...
function createMotionComponent({ preloadedFeatures: preloadedFeatures2, createVisualElement: createVisualElement2, useRender, useVisualState: useVisualState2, Component: Component3 }) {
  ...
  const ForwardRefComponent = (void 0)(MotionComponent);
...
var PresenceChild = ({ children, initial, isPresent: isPresent2, onExitComplete, custom, presenceAffectsLayout, mode }) => {
  ...
  if (mode === "popLayout") {
    children = (void 0)(PopChild, { isPresent: isPresent2, children });
  }
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, exitBeforeEnter, presenceAffectsLayout = true, mode = "sync" }) => {
  ...
  const forceRender = (void 0)(LayoutGroupContext).forceRender || useForceUpdate()[0];
  const isMounted = useIsMounted();
  const filteredChildren = onlyElements(children);
  let childrenToRender = filteredChildren;
  const exitingChildren = (void 0)(/* @__PURE__ */ new Map()).current;
  const presentChildren = (void 0)(childrenToRender);
  const allChildren = (void 0)(/* @__PURE__ */ new Map()).current;
  ...
  if (isInitialRender.current) {
    return (void 0)(void 0, { children: childrenToRender.map((child) => (void 0)(PresenceChild, { isPresent: true, initial: initial ? void 0 : false, presenceAffectsLayout, mode, children: child }, getChildKey(child))) });
  }
  ...
  exitingChildren.forEach((component, key) => {
    ...
    if (!exitingComponent) {
      ...
      exitingComponent = (void 0)(PresenceChild, { isPresent: false, onExitComplete: onExit, custom, presenceAffectsLayout, mode, children: child }, getChildKey(child));
  ...
  childrenToRender = childrenToRender.map((child) => {
    const key = child.key;
    return exitingChildren.has(key) ? child : (void 0)(PresenceChild, { isPresent: true, presenceAffectsLayout, mode, children: child }, getChildKey(child));
  });
  if (mode === "wait" && childrenToRender.length > 1) {
    console.warn(`You're attempting to animate multiple children within AnimatePresence, but its mode is set to "wait". This will lead to odd visual behaviour.`);
  }
  return (void 0)(void 0, { children: exitingChildren.size ? childrenToRender : childrenToRender.map((child) => (void 0)(child)) });
};
...
function MotionConfig({ children, isValidProp, ...config }) {
  ...
  const context = (void 0)(() => config, [JSON.stringify(config.transition), config.transformPagePoint, config.reducedMotion]);
...
function ReorderItem({ children, style = {}, value, as = "li", onDrag, layout: layout2 = true, ...props }, externalRef) {
  ...
  return (void 0)(Component3, { drag: axis, ...props, dragSnapToOrigin: true, style: { ...style, x: point2.x, y: point2.y, zIndex }, layout: layout2, onDrag: (event, gesturePoint) => {
    const { velocity } = gesturePoint;
    velocity[axis] && updateOrder(value, point2[axis].get(), velocity[axis]);
    onDrag && onDrag(event, gesturePoint);
  }, onLayoutMeasure: (measured) => registerItem(value, measured), ref: externalRef, ignoreStrict: true, children });
}
var Item = (void 0)(ReorderItem);
...
function useSpring(source, config = {}) {
  const { isStatic } = (void 0)(MotionConfigContext);
  const activeSpringAnimation = (void 0)(null);
  const value = useMotionValue(isMotionValue(source) ? source.get() : source);
  const latestValue = (void 0)(value.get());
  const latestSetter = (void 0)(() => {
  });
...
function useInView(ref, { root, margin, amount, once = false } = {}) {
  const [isInView, setInView] = (void 0)(false);
  (void 0)(() => {
